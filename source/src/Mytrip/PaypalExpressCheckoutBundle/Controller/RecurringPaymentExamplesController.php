<?php
namespace Mytrip\PaypalExpressCheckoutBundle\Controller;

use Mytrip\PaymentBundle\Model\AgreementDetails;
use Mytrip\PaymentBundle\Model\RecurringPaymentDetails;
...
class RecurringPaymentExamplesController extends PayumController
{
     ...
     *   name="mytrip_paypal_express_checkout_prepare_recurring_payment_agreement"
     * )
     *
     * @Extra\Template
     */
    public function createAgreementAction(Request $request)
    {
        ...
        if ($request->isMethod('POST')) {
            $storage = $this->getPayum()->getStorage('Mytrip\PaymentBundle\Entity\AgreementDetails');
            ...
            $captureToken = $this->getTokenFactory()->createCaptureToken(
                $paymentName,
                $agreement,
                'mytrip_paypal_express_checkout_create_recurring_payment'
            );
     ...
     *   name="mytrip_paypal_express_checkout_create_recurring_payment"
     * )
     */
    public function createRecurringPaymentAction(Request $request)
    {
        ...
        $storage = $this->getPayum()->getStorage('Mytrip\PaymentBundle\Entity\RecurringPaymentDetails');
        ...
        return $this->redirect($this->generateUrl('mytrip_paypal_express_checkout_view_recurring_payment', array(
     ...
     *   name="mytrip_paypal_express_checkout_view_recurring_payment"
     * )
     *
     * @Extra\Template
     */
    public function viewRecurringPaymentDetailsAction($paymentName, $billingAgreementId, $recurringPaymentId, Request $request)
    {
        ...
        $billingAgreementStorage = $this->getPayum()->getStorage('Mytrip\PaymentBundle\Entity\AgreementDetails');
        ...
        $recurringPaymentStorage = $this->getPayum()->getStorage('Mytrip\PaymentBundle\Entity\RecurringPaymentDetails');
        ...
        if ($recurringPaymentStatus->isSuccess()) {
            $cancelToken = $this->getTokenFactory()->createToken(
                ...
                'mytrip_paypal_express_checkout_cancel_recurring_payment',
                array(),
     ...
     *   name="mytrip_paypal_express_checkout_cancel_recurring_payment"
     * )
     */
