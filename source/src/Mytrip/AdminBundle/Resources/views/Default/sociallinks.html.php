...
<div id="content" class="clearfix"> 
    ...
    <div class="btnlink"></div> 	
        <div class="titletag"><h1>Social Links</h1></div>
    </div>
 ...
 <form name="searchfilters" action="" id="myForm1" method="post" style="width:800px;float:left;padding: 5px 10px;">  
        ...
        <td><input id="searchname" name="searchname" type="text" class="validate[groupRequired[payments]] text-input" autocomplete="off" value="<?php if(isset($_REQUEST['site'])){echo $_REQUEST['site'];}?>" /></td><td>&nbsp;</td>         
        <td><input type="submit" name="searchbutton" class="button small" value="Search" /></td>
            <td>&nbsp;</td><td>
		<?php if(isset($_REQUEST['search'])){	
		?>
        <a href="<?php echo $view['router']->generate('mytrip_admin_sociallinks');?>" class="button small" style="padding:3px 5px;">Cancel</a>
        <?php
		} ?></td>
        </tr></table></form>        
  </div>
<form action="" id="myForm" method="post">
    <table class="gtable sortable">
        ...
        <tr>
            ...
            <th align="left" class="name"><a <?php if(isset($pagerequest['direction'])){ echo ($pagerequest['sort']=="p.site"?($pagerequest['direction']=="asc"?'class="asc"':'class="desc"'):"");}?> href="?<?php echo $sortingrequest;?>&sort=p.site&direction=<?php if(isset($pagerequest['direction'])){ echo ($pagerequest['direction']=="asc" ? 'desc' : 'asc');}else{ echo "asc";}?>&page=<?php echo $paging['current'];?>" title="site">Website</a></th> 
             <th align="left" class="name">Link</th>          
		...
		foreach($pagination as $social){	
			?>
      <tr >
       <td align="center"><img src="<?php echo $view['assets']->getUrl('img/icons/arrow.jpg');?>"/></td>
    
        <td align="center"><?php echo $i; ?></td>
        <td align="left"><?php echo $social->getSite();?></td>   
        <td align="left"><?php echo $social->getLink();?></td>   
        <td align="center"><a href="<?php echo $view['router']->generate('mytrip_admin_editsociallink',array('id'=>$social->getSocialLinkId()));?>"><img src="<?php echo $view['assets']->getUrl('img/icons/edit.png');?>"/></a></td>       
      </tr>
      <?php $i++;
	  }
	  }?>
