...
<div id="content" class="clearfix"> 
    ...
    <div class="btnlink"><a href="<?php  echo $view['router']->generate('mytrip_admin_add_hostal');?>" class="button">Add Hostal</a></div> 	
        <div class="titletag"><h1>Hostal</h1></div>
    </div>
  <div class="tablefooter clearfix">
 <form name="searchfilters" action="<?php  echo $view['router']->generate('mytrip_admin_delete_hostal_multiple');?>" id="myForm1" method="post" style="width:800px;float:left;padding: 5px 10px;">  
		...
		?>
        <a href="<?php echo $view['router']->generate('mytrip_admin_list_hostal');?>" class="button small" style="padding:3px 5px;">Cancel</a>
        <?php
		} ?></td>
        </tr></table></form>        
  </div>
  
<form action="<?php  echo $view['router']->generate('mytrip_admin_delete_hostal_multiple') ?>" id="myForm" method="post">
    <table class="gtable sortable">
		...
		foreach($pagination as $hostallist){	
			?>
      <tr >
       <td align="center"><input type="checkbox" name="action[]" rel="action" value="<?php echo $hostallist->getHostalId(); ?>" /></td>
    
        <td align="center"><?php echo $i; ?></td>
        
        <td align="left"><?php echo $hostallist->getName();?></td>   
        
        <td align="left"><?php echo $hostallist->getStatus();?></td> 
        
                           
        <td align="left">
		<?php echo ($hostallist->getStatus()=="Active" ? '<a href="'.$view['router']->generate('mytrip_admin_status_hostal',array('page'=>'hostel','status'=>'inactive','id'=>$hostallist->getHostalId())).'" class="confirdel1" rel="Inactive">Click to Deactive</a>':'<a href="'.$view['router']->generate('mytrip_admin_status_hostal',array('page'=>'hostal','status'=>'active','id'=>$hostallist->getHostalId())).'" class="confirdel1" rel="Active">Click to Active</a>'); ?></td>       
         <td align="left"><a target="_blank"  href="<?php echo $view['router']->generate('mytrip_admin_manage_feature_hostal');?>?id=<?php echo $hostallist->getHostalId(); ?>"  >Manage Feature</a></td>   
 
 <td align="left"><a target="_blank" href="<?php echo $view['router']->generate('mytrip_admin_manage_banner_hostal'); ?>?id=<?php echo $hostallist->getHostalId(); ?>" >Manage Banner</a></td> 
        
        <td align="center"><a href="<?php  echo $view['router']->generate('mytrip_admin_edit_hostal',array('id'=>$hostallist->getHostalId()));?>"><img src="<?php echo $view['assets']->getUrl('img/icons/edit.png');?>"/></a></td>
        
        <td align="center"><a href="<?php echo $view['router']->generate('mytrip_admin_delete_hostal',array('id'=>$hostallist->getHostalId()));?>" class="confirdel"><img src="<?php echo $view['assets']->getUrl('img/icons/cross.png');?>"/></a></td>
      </tr>
      <?php $i++;
	  }
	  }?>
