...
<div id="content" class="clearfix"> 
    ...
    <div class="btnlink"><a href="<?php echo $view['router']->generate('mytrip_admin_add_cancel_setting');?>" class="button">Add Cancel Settings</a></div> 	
        <div class="titletag"><h1>Cancel Settings</h1></div>
    </div>
  <div class="tablefooter clearfix">
     &nbsp;   
  </div>
<form action="<?php echo $view['router']->generate('mytrip_admin_delete_cancel_settings') ?>" id="myForm" method="post">
    <table class="gtable sortable">
		...
		foreach($pagination as $settings){	
			?>
      <tr >
       <td align="center"><?php if($settings->getCancelId()==1){?><img src="<?php echo $view['assets']->getUrl('img/icons/arrow.jpg');?>"/><?php }else{ ?><input type="checkbox" name="action[]" rel="action" value="<?php echo $settings->getCancelId(); ?>" /><?php }?></td>
        <td align="center"><?php echo $i; ?></td>
        <td align="left"><?php echo $settings->getDays();?> days</td>
        <td align="left"><?php echo $settings->getPercentage();?> %</td>      
        <td align="center"><a href="<?php echo $view['router']->generate('mytrip_admin_edit_cancel_settings',array('id'=>$settings->getCancelId()));?>"><img src="<?php echo $view['assets']->getUrl('img/icons/edit.png');?>"/></a></td>
        <td align="center"><?php if($settings->getCancelId()>1){?><a href="<?php echo $view['router']->generate('mytrip_admin_delete_cancel_settings',array('id'=>$settings->getCancelId()));?>" class="confirdel"><img src="<?php echo $view['assets']->getUrl('img/icons/cross.png');?>"/></a><?php }?></td>
      </tr>
      <?php $i++;
	  }
	  }?>
