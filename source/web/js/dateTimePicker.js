...
(function(factory) {
  if (typeof define === 'function' && define.amd) {
	...
	Plugin.prototype = {
		...
		update: function(){
			...
			if (typeof this.options.adapter === 'string'){
				$.getJSON(this.options.adapter, {
					...
					_this.unavailable = _this.options.unavailable.concat(data.unavailable);
					_this.toConfirm = _this.options.toConfirm.concat(data.toconfirm);
					_this.draw(months);
				});
			}else{
				this.unavailable = this.options.unavailable;
				this.toConfirm = this.options.toConfirm;
				this.draw(months);
			}
		},
										...
										ap.push($('<td>').addClass(function(){
											...
											if (_this.isAvailable(d.getDate(), d.getMonth() + 1, d.getFullYear())){
												if (d.getMonth() == date.getMonth()){													
													if (_this.isConfirm(d.getDate(), d.getMonth() + 1, d.getFullYear())){
														if (_this.isPast(d.getDate(), d.getMonth() + 1, d.getFullYear())){
															cls.push('topast');
														}else{
															cls.push('toconfirm');
														}
											...
											return cls.join(' ');											
										}).data({date: d.getMonth() == date.getMonth()?d.getDate():'', month: d.getMonth() == date.getMonth()?d.getMonth() + 1:'', year: d.getMonth() == date.getMonth()?d.getFullYear():''}).append( d.getMonth() == date.getMonth()?(d.getDate()<10?'<span class="adate">0'+d.getDate()+'</span>':'<span class="adate">'+d.getDate()+'</span>'):'').click(function(){
											if($(this).data!=''){
		...
		isConfirm: function(date, month, year){
			for (var i in this.toConfirm){				
				var book_date = this.toConfirm[i].split('-');
				if (book_date.length !== 3){
					return false;
				}else if (
					(book_date[0] == '*' || book_date[0] - year === 0)	&&
		...
		isPast: function(date, month, year){
			var curs_date = new Date();
			//for (var i in this.toConfirm){				
				var book_date = new Date(year,month-1,date);
				/*if (book_date.length !== 3){
					return false;
				}else*/ if (curs_date < book_date){
					return false;
				}
			//}
			return true;
